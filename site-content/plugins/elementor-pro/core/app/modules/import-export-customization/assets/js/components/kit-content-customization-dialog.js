...
export function KitContentCustomizationDialog( {
	...
	useEffect( () => {
		...
		setSettings( ( prevSettings ) => ( {
			...prevSettings,
			menus: isImport
				? ( data.customization.content?.menus || Object.keys( data?.uploadedData?.manifest[ 'site-content' ]?.nav_menu_item || {} ).length > 0 )
	...
	const isMenusExported = () => {
		return Object.keys( data?.uploadedData?.manifest?.[ 'site-content' ]?.nav_menu_item || {} ).length > 0 ||
