...
function KitContentCustomizationDialog({
  ...
  (0, _react.useEffect)(() => {
    ...
    setSettings(prevSettings => ({
      ...prevSettings,
      menus: isImport ? data.customization.content?.menus || Object.keys(data?.uploadedData?.manifest['site-content']?.nav_menu_item || {}).length > 0 : data.customization.content?.menus ?? initialState
    }));
  ...
  const isPagesExported = () => {
    ...
    const wpContent = data?.uploadedData?.manifest?.['site-content'];
  ...
  const isMenusExported = () => {
    return Object.keys(data?.uploadedData?.manifest?.['site-content']?.nav_menu_item || {}).length > 0 || customPostTypes?.find(cpt => cpt.value.includes('nav_menu'));
  };
...
function useKitCustomizationPages({
  ...
  const pageOptions = (0, _react.useMemo)(() => {
    ...
    const wpPages = Object.entries(data?.uploadedData?.manifest?.['site-content']?.page || {}).map(([id, page]) => {
      return {
        value: id,
        label: page.title
      };
